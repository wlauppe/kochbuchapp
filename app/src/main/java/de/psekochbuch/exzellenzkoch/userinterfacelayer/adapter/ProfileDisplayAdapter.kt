...
class ProfileDisplayAdapter (viewModel: ProfileDisplayViewmodel) :RecyclerView.Adapter<ProfileDisplayAdapter.ProfileDisplayViewHolder>() {
    //Attributes delclareted
    var navController: NavController? = null

    var viewmModel: ProfileDisplayViewmodel = viewModel
    var names = viewModel.recipes

    //OnCreateViewHolder inflates the layout and sets binding & Navcontroller
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProfileDisplayAdapter {
    ...
    override fun onBindViewHolder(holder: ProfileDisplayAdapter, position: Int) {
        if (names.value.isNullOrEmpty()) {
        ...
        holder..textViewRecipeTitleItem.text = param
        //Set the onClicklistener on the Buttons which use the navcontroller to change to another fragment
        holder.profileRecipeListItemBinding.buttonEditRecipe.setOnClickListener {
        ...
        holder..buttonRemoveRecipe.setOnClickListener {
    ...
    class ProfileDisplayViewHolder(val profileRecipeListItemBinding: ProfileDisplayRecipeItemBinding) :
        RecyclerView.ViewHolder(profileRecipeListItemBinding.root)
